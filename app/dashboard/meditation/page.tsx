...
export default function MeditationPage() {
  ...
  const [stats, setStats] = useState({
    ...
    longestSession: 0,
    last7Days: [] as { date: string; minutes: number }[],
    recentSessions: [] as MeditationSession[],
  });

  const intervalRef = useRef<NodeJS.Timeout | null>(null);
  const audioRef = useRef<HTMLAudioElement | null>(null);

  // Fetch meditation sessions and stats
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        toast({
          title: "Error",
          description: "Failed to load meditation data",
          variant: "destructive",
        });
      } finally {
        setIsLoading(false);
  ...
  // Initialize audio
  useEffect(() => {
    audioRef.current = new Audio("/sounds/meditation-sound.mp3"); // You would need to add this file
  ...
  const handleTimerComplete = () => {
    ...
    const completionSound = new Audio("/sounds/completion-sound.mp3"); // You would need to add this file
    completionSound
      .play()
    ...
    toast({
      ...
      description: `You've completed a ${formatTime(
        targetDuration
      )} meditation session.`,
    });
  };
  ...
  const saveSession = async () => {
    ...
    try {
      const result = await saveMeditationSession(
        timeElapsed,
        sessionType,
      ...
      toast({
        title: "Session saved",
        description: `Your ${formatTime(
          timeElapsed
        )} meditation has been recorded.`,
      });
    } catch (error) {
  ...
  const totalMeditationHours = Math.floor(stats.totalTime / 3600);
  const totalMeditationMinutes = Math.floor((stats.totalTime % 3600) / 60);
                ...
                <div className="absolute inset-0">
                  <svg className="h-full w-full" viewBox="0 0 100 100">
                    ...
                    <circle
                      ...
                      strokeDashoffset={
                        301.59 -
                        (301.59 * (timeElapsed / targetDuration) * 100) / 100
                      }
        ...
        <Card>
          ...
          <CardContent>
            <ChartContainer
              ...
              className="h-[300px]"
            >
              <BarChart accessibilityLayer data={stats.last7Days}>
              ...
              <div className="space-y-2">
                ...
                <p className="text-2xl font-bold">
                  {stats.totalSessions > 0
                    ? Math.floor(stats.averageSessionDuration / 60)
                    : 0}
                  m
                </p>
              </div>
