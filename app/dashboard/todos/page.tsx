...
import { CalendarIcon, Check, Clock, Edit, Loader2, Plus, Tag, Trash } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { getTodos, createTodo, updateTodo, deleteTodo, toggleTodoCompletion } from "@/lib/actions/todos"
...
export default function TodosPage() {
  ...
  const filteredTodos = todos.filter((todo) => {
    ...
    if (activeTab === "today" && (!todo.dueDate || !isToday(new Date(todo.dueDate)))) return false
    if (activeTab === "upcoming" && (!todo.dueDate || !isAfter(new Date(todo.dueDate), new Date()))) return false
    if (activeTab === "overdue" && (!todo.dueDate || !isBefore(new Date(todo.dueDate), new Date()) || todo.completed))
      return false
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Select value={newTodoPriority} onValueChange={(value) => setNewTodoPriority(value)}>
      ...
      <div className="grid gap-6 md:grid-cols-[250px_1fr]">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {isLoading ? "Loading..." : `${filteredTodos.length} ${filteredTodos.length === 1 ? "task" : "tasks"}`}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        {todo.description && (
                          <p
                            className={`text-sm ${
                              todo.completed ? "text-muted-foreground/70 line-through" : "text-muted-foreground"
                            }`}
                          >
                            {todo.description}
                          </p>
              ...
              <div className="grid gap-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {editingTodo.dueDate ? format(new Date(editingTodo.dueDate), "PPP") : "Select a date"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar
